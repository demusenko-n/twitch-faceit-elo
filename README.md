...
- Successful responses are edge-cached (`/elo` 30s, `/maxelo` 1h, `/playerid` 24h) to spare the upstream APIs. The cache is per-datacenter and errors are never cached, so data can lag by up to the route's TTL.
